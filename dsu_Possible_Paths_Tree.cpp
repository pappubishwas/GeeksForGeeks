...
class Solution{
    ...
    int Union(int a, int b, vector<int> &parent, vector<int> &sz)
    {
	    ...
	    ans += sz[ra]*sz[rb];  // number of paths less then or equal to current weights 
	    parent[rb] = ra;
	    sz[ra] += sz[rb];
	...
	vector<int> maximumWeight(int n, vector<vector<int>> edges, int q, vector<int> &queries)
	{
        ...
        for(int i = 0; i < q; i++)
        {
            ...
            if(val == mp.begin())
                res.push_back(0); //If there is no such element for query[i] weights, then the maximum paths is 0.
            else
            {
                val--;
                res.push_back(val->second); //Storing the maximum paths for the query[i] weights.
            }
       	}
       	return res;
	}
